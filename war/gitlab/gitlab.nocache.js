...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "gitlab" because\n';
  ...
  $wnd.__gwt_activeModules['gitlab'] = {
    'moduleName' : 'gitlab',
    'bindings' : function() {
      return {};
    }
 ...
 * "/gitlab.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['gitlab'].superdevmode = true;

  var expectedSuffix = '/gitlab.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + gitlab + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:gitlab';
  var devModeSessionKey = '__gwtDevModeSession:gitlab';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/gitlab/gitlab.nocache.js';
  ...
  $wnd.__gwt_activeModules['gitlab'].canRedirect = true;
  $wnd.__gwt_activeModules['gitlab'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/gitlab?user.agent=' + ua + '&_callback=' + callback;
